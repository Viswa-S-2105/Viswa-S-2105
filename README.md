<h1 align="center">Hey 👋 I'm Viswa</h1>
<p align="center">
🎓 CSE @ CEG, Anna University | 💻 Developer | 🌱 Lifelong Learner  
</p>

---

### 👨‍💻 About Me

I’m a passionate Computer Science Engineering student in my Final year.  
I love turning ideas into real-world applications and collaborating on meaningful projects.

---

### 🚀 Tech Stack

**Languages:** Java, C, C++, Python, HTML/CSS, JavaScript, SQL  
**Technologies/Frameworks:** React, Node.js, Express, MongoDB, MySQL 

---

### 📌 Highlight Projects

🔹 **[Explainable Compromised Insider Threat Detection](https://github.com/Viswa-S-2105/Explainable-Insider-Threat-Detection)**  
> An explainable AI-based insider threat detection system using IDS-driven user behaviour analytics and attention-based deep learning

🔹 **[AI-Driven-MSME-Lead-Matchmaker](https://github.com/Viswa-S-2105/AI-Driven-MSME-Lead-Matchmaker)**  
> Bridging the digital divide in B2B trade. An AI matchmaker that uses NLP to automatically route global export inquiries to Indian MSMEs via SMS and Email.

🔹 **[Events Management System](https://github.com/Viswa-S-2105/Events-Management-Application)**  
> MERN stack web app to manage university-level events with certificate generation, dashboards, and route protection.

---

### 🌐 Let's Connect

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Viswa-blue?style=flat&logo=linkedin)](https://linkedin.com/in/viswa2105)

<!--
**Viswa-S-2105/Viswa-S-2105** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
